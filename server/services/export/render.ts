...
export function buildExportHtml(
  ...
  const markdownSource = buildExportMarkdown(curProject, wikiData, list, true);
  const renderer = markdownIt({ html: true, breaks: true });
  renderer.use(markdownItAnchor);
  renderer.use(markdownItTableOfContents, {
    markerPattern: /^\[toc\]/im,
  });

  let rendered = unescape(renderer.render(markdownSource));
  let left;
  ...
  return wrapHtmlDocument(curProject, themeCss, left || "", content);
}

export function safeBuildExport(
  buildFn: () => string
): { ok: true; body: string } | { ok: false; message: string } {
  try {
    return { ok: true, body: buildFn() };
  } catch (e) {
    appLog(e, "error");
    return { ok: false, message: "下载出错" };
  }
}
