...
const initPlugins = require("./common/plugin").initPlugins;
let extConfig = require("./common/config").exts;
...
pluginsConfig.forEach((plugin) => {
  ...
  if (
    !yapi.commons.fileExist(
      yapi.path.join(plugin_path, "yapi-plugin-" + plugin.name + "/server")
    )
  ) {
  ...
  let pluginModule = require(yapi.path.join(
    plugin_path,
    "yapi-plugin-" + plugin.name + "/server"
  ));
...
extConfig.forEach((plugin) => {
  ...
  if (
    !yapi.commons.fileExist(
      yapi.path.join(plugin_system_path, "yapi-plugin-" + plugin.name + "/server")
    )
  ) {
    throw new Error(`config.json配置了插件${plugin},但plugins目录没有找到此插件，请安装此插件`);
  }
