...
exports.initialize = function(connection) {
  try {
    ...
    console.error("MissingSchemaError 111111111", ex);
    ...
    if (that.isNew) {
      ...
      (function save() {
        ...
        if (ready) {
          ...
          if (typeof that[settings.field] === "number") {
            IdentityCounter.findOneAndUpdate(
              ...
              { model: settings.model, field: settings.field, count: { $lt: that[settings.field] } },
              // Change the count of the value found to the new field value.
              { count: that[settings.field] },
              function(err) {
                if (err) {
                  return next(err);
                }
            ...
            IdentityCounter.findOneAndUpdate(
              ...
              function(err, updatedIdentityCounter) {
                ...
                that[settings.field] = updatedIdentityCounter.count;
                // Continue with default document save functionality.
                next();
              }
            );
