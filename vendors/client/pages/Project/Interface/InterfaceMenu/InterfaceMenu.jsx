...
class InterfaceMenu extends Component {
  ...
  handleAddInterfaceCat = (data) => {
    data.project_id = this.props.projectId;
    request.post("/interface/add_cat", data).then((res) => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
  ...
  handleChangeInterfaceCat = (data) => {
    ...
    request.post("/interface/up_cat", params).then((res) => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
  ...
  copyInterface = async(id) => {
    ...
    request.post("/interface/add", newData).then(async(res) => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
      message.success("接口添加成功");
  ...
  onDrop = async(e) => {
    ...
    if (id.indexOf("cat") === -1) {
      if (dropCatId === dragCatId) {
        ...
        request.post("/interface/up_index", changes).then();
      } else {
        await request.post("/interface/up", { id, catid: dropCatId });
      }
      ...
      request.post("/interface/up_cat_index", changes).then();
