/**
 * 获取所需要的日期区间点
 * @param time {Number} Number是ele日期区间选择组件返回的结果，Number是之前时刻距离今天的间隔天数，默认是90天
 ...
 */
const getDateRange = (time = 90, start = false, withToday = true) => {
...
/**
 * 格式化 年、月、日、时、分、秒
 * @param {Object|String|Number}  val 日期对象 或是可new Date的对象或时间戳
 ...
 * @param dateRange
 */
const getDayGapFromRange = (dateRange) => {
 ...
 * @returns {string}
 */
const transformBytesToGB = (bytes) => (bytes / 1024 / 1024 / 1024).toFixed(2);
/**
 * @param seconds
 * @returns {string}
 */
const transformSecondsToDay = (seconds) => (seconds / 3600 / 24).toFixed(2);
//
module.exports = {
  ...
  getDayGapFromRange: getDayGapFromRange,
  transformBytesToGB: transformBytesToGB,
  transformSecondsToDay: transformSecondsToDay
}