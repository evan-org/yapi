...
class MockCol extends Component {
  ...
  state = {
    caseData: {}, caseDesModalVisible: false, isAdd: false
  };
  constructor(props) {
    super(props);
  }
  ...
  openModal = (record, isAdd) => async() => {
    if (this.props.currInterface.res_body_is_json_schema && isAdd) {
      let result = await axios.post("/api/interface/schema2json", {
        schema: json5_parse(this.props.currInterface.res_body), required: true
      });
    ...
    if (this.props.currInterface.req_body_is_json_schema) {
      let result = await axios.post("/api/interface/schema2json", {
        schema: json5_parse(this.props.currInterface.req_body_other), required: true
      });
      record.req_body_other = JSON.stringify(result.data);
    }
    this.setState({
      isAdd: isAdd, caseDesModalVisible: true, caseData: record
    });
  };
  handleOk = async(caseData) => {
    ...
    caseData = Object.assign({
      ...caseData, interface_id: interface_id, project_id: project_id
    });
    if (!this.state.isAdd) {
  ...
  render() {
    ...
    const initCaseData = {
      ip: "", ip_enable: false, name: currInterface.title, code: "200", delay: 0, headers: [{ name: "", value: "" }], params: {}, res_body: currInterface.res_body
    };
    let ipFilters = [];
    let ipObj = {};
    let userFilters = [];
    ...
    const columns = [{
      title: "期望名称", dataIndex: "name", key: "name"
    }, {
      title: "ip", dataIndex: "ip", key: "ip", render: (text, recode) => {
        if (!recode.ip_enable) {
          text = "";
        }
        return text;
      }, onFilter: (value, record) => (record.ip === value && record.ip_enable) || (value === "无过滤" && !record.ip_enable), filters: ipFilters
    }, {
      title: "创建人", dataIndex: "username", key: "username", onFilter: (value, record) => record.username === value, filters: userFilters
    }, {
      title: "编辑时间", dataIndex: "up_time", key: "up_time", render: (text) => formatTime(text)
    }, {
      title: "操作", dataIndex: "_id", key: "_id", render: (_id, recode) =>
        // console.log(recode)
        (!isGuest && (<div>
    ...
    return (<div>
      ...
      {caseDesModalVisible && (<CaseDesModal
