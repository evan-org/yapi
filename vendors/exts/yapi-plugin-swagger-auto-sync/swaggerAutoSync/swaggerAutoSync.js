...
const formItemLayout = {
  labelCol: {
    lg: { span: 5 }, xs: { span: 24 }, sm: { span: 10 }
  }, wrapperCol: {
    lg: { span: 16 }, xs: { span: 24 }, sm: { span: 12 }
  }, className: "form-item"
};
...
class ProjectInterfaceSync extends Component {
  ...
  handleSubmit = async() => {
    ...
    let params = {
      project_id: projectId, is_sync_open: this.state.sync_data.is_sync_open, uid: this.props.projectMsg.uid
    };
  ...
  render() {
    ...
    return (<div className="m-panel">
      <Form>
        ...
        <div>
          <FormItem {...formItemLayout} label={<span className="label">
                数据同步&nbsp;
            <Tooltip
              title={<div>
            ...
            {getFieldDecorator("sync_mode", {
              initialValue: this.state.sync_data.sync_mode, rules: [{
                required: true, message: "请选择同步方式!"
              }]
            })(<Select>
          ...
          <FormItem {...formItemLayout} label="项目的swagger json地址">
            {getFieldDecorator("sync_json_url", {
              rules: [{
                required: true, message: "输入swagger地址"
              }, {
                validator: this.validSwaggerUrl
              }], validateTrigger: "onBlur", initialValue: this.state.sync_data.sync_json_url
            })(<Input/>)}
          </FormItem>
