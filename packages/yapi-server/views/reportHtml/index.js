...
function createHtml(reports) {
  ...
  reports.list.map((item, index) => {
    ...
    mdTemplate += responseHtml(item.res_header, item.res_body);
...
function responseHtml(res_header, res_body) {
...
function validHtml(validRes) {
  if (validRes && Array.isArray(validRes)) {
    validRes = validRes.map((item, index) => `<div data-key="${index}">${item.message}</div>`);
  }
  return `
    <div>
      <div class="row case-report">
