...
- 返回数据分为 `json` & `raw` 两种形式。基于 mockjs（详见 [Mock 介绍](./mock.md)）和 json5，使用注释方式写参数说明
...
- 开放接口：默认为关闭状态；导出时可在 **项目设置 → 环境与数据** 选择仅导出公开接口


## 接口调试

在接口详情页切换到 **调试** Tab，选择项目环境后点击 **发送请求**，即可对真实后端发起 HTTP 请求（类似 Postman）。

- 请先在 **项目设置 → 环境与数据** 配置环境域名
- 浏览器直接请求可能受目标站点 CORS 限制；Mock 请访问 `/mock/{projectId}{path}`
- 可将当前接口保存到测试集合，便于后续批量测试
